...
export class PlaywrightBinary extends AbstractBinary {
  ...
  async fetch(dir: string): Promise<FetchResult | undefined> {
    if (!this.dirItems) {
      ...
      const packageVersionsMap = new Map(Object.keys(packageData.versions)
        .filter(version => version.match(/^(?:\d+\.\d+\.\d+)(?:-beta-\d+)?$/))
        .map(version => [ version, packageData.time[version] as string ]));
      const browsersMap = new Map<string, { name: string; revision: string; revisionOverrides?:Record<string, string> }[]>();
      await Promise.all(
        [ ...packageVersionsMap.keys() ].map(version =>
          this.requestJSON(
            `https://unpkg.com/playwright-core@${version}/browsers.json`,
          )
            .then(data => browsersMap.set(version, data.browsers))
            .catch(error =>
              this.logger.warn(`Playwright version ${version} browser data request failed:`, error),
            ),
        ),
      );

      for (const [ version, browsers ] of browsersMap) {
        for (const browser of browsers) {
          const downloadPaths = DOWNLOAD_PATHS[browser.name];
          if (!downloadPaths) return;

          for (const [ platform, remotePath ] of Object.entries(downloadPaths)) {
            if (typeof remotePath !== 'string') continue;
            const revision = browser.revisionOverrides && browser.revisionOverrides[platform] ?
              browser.revisionOverrides[platform] :
              browser.revision;
            const url = DOWNLOAD_HOST + util.format(remotePath, revision),
              name = path.basename(remotePath),
              date = packageVersionsMap.get(version),
              dir = `/builds/${browser.name}/${revision}/`;
            if (!date) continue;
            if (!this.dirItems[dir]) {
              this.dirItems[`/builds/${browser.name}/`].push({ name: `${revision}/`, isDir: true, url: '', size: '-', date });
              this.dirItems[dir] = [];
            }
            if (!this.dirItems[dir].find(item => item.name === name)) {
              this.dirItems[dir].push({ name, isDir: false, url, size: '-', date });
            }
          }
        }
      }
