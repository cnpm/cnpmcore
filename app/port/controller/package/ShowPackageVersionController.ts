...
export class ShowPackageVersionController extends AbstractController {
  ...
  async show(@HTTPContext() ctx: Context, @HTTPParam() fullname: string, @HTTPParam() versionSpec: string) {
    ...
    const { blockReason, manifest, pkg } = await this.packageManagerService.showPackageVersionManifest(
      scope,
      name,
      versionSpec,
      isSync,
