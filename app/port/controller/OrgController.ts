...
export class OrgController extends AbstractController {
  ...
  async listMembers(@Context() ctx: EggContext, @HTTPParam() orgName: string) {
    ...
    // npm org ls returns { username: role }
    const users = await Promise.all(
      members.map(m => this.userRepository.findUserByUserId(m.userId)),
    );
  ...
  // GET /-/org/:orgName/member/:username/team — list teams a user belongs to in this org
  @HTTPMethod({
