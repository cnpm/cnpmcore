...
import { UpstreamRepository } from '../../repository/UpstreamRepository';
...
export class NpmUpstreamSyncer {
  ...
  @Inject()
  private readonly upstreamRepository: UpstreamRepository;

  async listenReplicate() {
    let lastChangeSeq = 0;
    const lastChange = await this.upstreamRepository.findLastUpstreamChange();
    if (lastChange) {
      lastChangeSeq = lastChange.seq;
    }
