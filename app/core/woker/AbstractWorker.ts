...
export abstract class AbstractWorker {
  ...
  async initWorkerInfo() {
    throw new Error('initWorkerInfo not implemented.');
  }
  ...
  async registerWorker() {
    ...
    this.worker.on('failed', (job?: Job) => {
      if (job) {
        this.app.logger.info(`[${this.queueKey}_worker:subscribe:executeTask:failed] taskId: %s, targetName: %s, attemptsMade %s, reason: %s`,
          job.data.taskId, job.data.targetName, job.attemptsMade, job.failedReason);
      }
    });
