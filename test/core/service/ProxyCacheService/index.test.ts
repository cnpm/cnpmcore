...
describe('test/core/service/ProxyCacheService/index.test.ts', () => {
  ...
  describe('getPackageVersionTarBuffer()', () => {
    it('should get tgz buffer from source', async () => {
      const data = await TestUtil.readFixturesFile('registry.npmjs.org/foobar/-/foobar-1.0.0.tgz');
      app.mockHttpclient('https://registry.npmjs.org/foobar/-/foobar-1.0.0.tgz', 'GET', {
        data,
        persist: false,
      });
      const buffer = await proxyCacheService.getPackageVersionTarBuffer('foobar', 'foobar/-/foobar-1.0.0.tgz');
    ...
    it('should block package in block list', async () => {
      ...
      try {
        await proxyCacheService.getPackageVersionTarBuffer('bar', 'bar/-/bar-1.0.0.tgz');
      } catch (error) {
        assert.equal(error, 'ForbiddenError: stop proxy by block list: ["bar"]');
      }
    });
  });
  ...
  describe('getSourceManifestAndCache()', () => {
    it('should get full package manifest', async () => {
      const data = await TestUtil.readJSONFile(TestUtil.getFixtures('registry.npmjs.org/foobar.json'));
    ...
    it('should get full package version manifest', async () => {
      const data = await TestUtil.readJSONFile(TestUtil.getFixtures('registry.npmjs.org/foobar/1.0.0/package.json'));
  ...
  describe('getPackageVersionManifest()', () => {
    ...
    it('should get correct verison via tag and cache the pkg manifest', async () => {
      ...
      const pkgVersionManifest = await proxyCacheService.getPackageVersionManifest('foobar', DIST_NAMES.MANIFEST, 'latest');
      assert(pkgVersionManifest);
      assert.equal(pkgVersionManifest.version, '1.1.0');
      const proxyCacheRepository = await app.getEggObject(ProxyCacheRepository);
      const pkgManifest = proxyCacheRepository.findProxyCache('foobar', DIST_NAMES.ABBREVIATED_MANIFESTS);
      assert(pkgManifest);
    });
  });
