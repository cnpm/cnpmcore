...
describe('test/common/adapter/binary/NodePreGypBinary.test.ts', () => {
  ...
  describe('fetch()', () => {
    ...
    it('should fetch @discordjs/opus', async () => {
      app.mockHttpclient('https://registry.npmjs.com/@discordjs/opus', 'GET', {
        data: {
          versions: {
            '0.10.0': {
              name: '@discordjs/opus',
              binary: {
                ...
                package_name:
                  '{module_name}-v{version}-{node_abi}-napi-v{napi_build_version}-{platform}-{arch}-{libc}-{libc_version}.tar.gz',
                napi_versions: [3],
              },
            },
          },
          time: {
            '0.10.0': '2025-01-25T21:10:59.000Z',
          },
        },
      });
      app.mockHttpclient('https://nodejs.org/dist/index.json', 'GET', {
        data: [
          { version: 'v17.9.1', modules: '102' },
          { version: 'v18.20.0', modules: '108' },
        ],
      });
      ...
      for (const item of result.items) {
        ...
        if (item.name === 'v0.10.0/') {
          assert.equal(item.date, '2025-01-25T21:10:59.000Z');
      ...
      for (const item of result.items) {
        ...
        if (item.name === 'opus-v0.10.0-node-v102-napi-v3-linux-arm64-musl-1.2.2.tar.gz') {
          assert.equal(item.date, '2025-01-25T21:10:59.000Z');
          assert.equal(item.size, '-');
          assert.equal(
            item.url,
            'https://github.com/discordjs/opus/releases/download/v0.10.0/opus-v0.10.0-node-v102-napi-v3-linux-arm64-musl-1.2.2.tar.gz',
          );
          matchFile3 = true;
        }
        ...
        if (item.name === 'opus-v0.10.0-node-v108-napi-v3-win32-ia32-unknown-unknown.tar.gz') {
