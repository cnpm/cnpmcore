  ...
  describe('fetch()', () => {
    it('should fetch root: / work', async () => {
      ...
      for (const item of result.items) {
        ...
        if (item.name === 'node-v0.1.100.tar.gz') {
          ...
          assert.ok(item.url === 'https://nodejs.org/dist/node-v0.1.100.tar.gz');
          matchFile = true;
        }
        if (!item.isDir) {
    ...
    it('should fetch subdir: /v16.13.1/ work', async () => {
      ...
      for (const item of result.items) {
        ...
        if (item.name === 'SHASUMS256.txt') {
          ...
          assert.ok(item.url === 'https://nodejs.org/dist/v16.13.1/SHASUMS256.txt');
          matchFile = true;
        }
        if (!item.isDir) {
          assert.ok(typeof item.size === 'string');
    ...
    it('should fetch subdir: /v20.19.5/ work', async () => {
      app.mockHttpclient('https://nodejs.org/dist/v20.19.5/', 'GET', {
        data: await TestUtil.readFixturesFile('nodejs.org/site/v20.19.5/index.html'),
      });
      ...
      for (const item of result.items) {
        ...
        if (item.name === 'SHASUMS256.txt') {
          ...
          assert.ok(item.url === 'https://nodejs.org/dist/v20.19.5/SHASUMS256.txt');
          matchFile = true;
        }
        if (!item.isDir) {
          assert.ok(typeof item.size === 'string');
    ...
    it('should fetch subdir: /v14.0.0-nightly20200119b318926634/ work', async () => {
      ...
      for (const item of result.items) {
        ...
        if (item.name === 'SHASUMS256.txt') {
          ...
          assert.ok(
            item.url === 'https://nodejs.org/download/nightly/v14.0.0-nightly20200119b318926634/SHASUMS256.txt',
          );
          matchFile1 = true;
        }
    ...
    it('should skip zero size file', async () => {
      ...
      for (const item of result.items) {
        ...
        if (item.name === 'SHASUMS256.txt') {
          ...
          assert.ok(
            item.url === 'https://nodejs.org/download/nightly/v14.0.0-nightly20200204ee9e689df2/SHASUMS256.txt',
          );
          matchFile1 = true;
        }
    ...
    it('should on python', async () => {
      ...
      for (const item of result.items) {
        ...
        if (item.name === 'README.html') {
          ...
          assert.ok(item.isDir === false);
          assert.ok(item.size);
          assert.ok(item.url === 'https://www.python.org/ftp/python/README.html');
          matchFile = true;
        }
        if (!item.isDir) {
          assert.ok(typeof item.size === 'string');
      ...
      for (const item of result.items) {
        ...
        if (item.name === 'python-3.7.3.exe') {
          ...
          assert.ok(item.url === 'https://www.python.org/ftp/python/3.7.3/python-3.7.3.exe');
          matchFile = true;
        }
        if (!item.isDir) {
          assert.ok(typeof item.size === 'string');
