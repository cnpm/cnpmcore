...
describe('test/port/controller/PackageVersionFileController/raw.test.ts', () => {
  ...
  describe('[GET /:fullname/:versionSpec/files/:path] raw()', () => {
    it('should show one package version raw file', async () => {
      ...
      res = await app.httpRequest().get(`/${pkg.name}/latest/files/package.json`);
      ...
      res = await app.httpRequest().get(`/${pkg.name}/^1.0.0/files/package.json`);
      assert.equal(res.status, 302);
      assert.equal(res.headers.location, `/${pkg.name}/1.0.0/files/package.json`);
      assert.equal(res.headers['cache-control'], 'public, s-maxage=600, max-age=60');
      assert.equal(res.headers.vary, 'Origin, Accept, Accept-Encoding');

      res = await app.httpRequest().get(`/${pkg.name}/%5E1.0.0/files/package.json`);
      assert.equal(res.status, 302);
      assert.equal(res.headers.location, `/${pkg.name}/1.0.0/files/package.json`);
      assert.equal(res.headers['cache-control'], 'public, s-maxage=600, max-age=60');
      assert.equal(res.headers.vary, 'Origin, Accept, Accept-Encoding');
    ...
    it('should show one package version file meta', async () => {
      ...
      res = await app.httpRequest().get(`/${pkg.name}/latest/files/package.json?meta=2`);
      assert.equal(res.status, 302);
      ...
      res = await app.httpRequest().get('/foo/1.0.0/files/package2.json?meta').expect(404);
    ...
    it('should 422 when invalid spec', async () => {
      ...
      const res = await app.httpRequest().get('/foo/@invalid-spec/files/package.json?meta').expect(422);
    ...
    it('should support non-ascii file name', async () => {
      ...
      assert.equal(res.status, 200);
      // console.log(res.body);
      assert.ok(res.body.files.find((file: { path: string }) => file.path === '/resource/ToOneFromχ.js'));
    ...
    it('should support non-npm pack tgz file', async () => {
      ...
      res = await app.httpRequest().get(`/${pkg.name}/1.0.0/files/`);
      assert.equal(res.status, 200);
      assert.ok(res.body.files.find((file: { path: string }) => file.path === '/package.json'));
      res = await app.httpRequest().get(`/${pkg.name}/1.0.0/files/zipObjectDeep.d.ts`);
      assert.equal(res.status, 200);
    ...
    it('should ignore "." hidden dir', async () => {
      ...
      assert.ok(res.body.files.find((file: { path: string }) => file.path === '/dist'));
    ...
    it('should handle big tgz file', async () => {
      ...
      assert.ok(res.body.files.find((file: { path: string }) => file.path === '/CONTRIBUTING.md'));
      ...
      res = await app.httpRequest().get(`/${pkg.name}/1.0.0/files/lib/index.js`);
      assert.equal(res.status, 200);
      assert.equal(res.headers['cache-control'], 'public, max-age=31536000');
      assert.equal(res.headers.vary, 'Origin, Accept, Accept-Encoding');
      assert.equal(res.headers['content-type'], 'text/javascript; charset=utf-8');
      ...
      res = await app.httpRequest().get(`/${pkg.name}/1.0.0/files/docs/_site/getting-started.html?meta`);
      assert.equal(res.status, 200);
      assert.equal(res.headers['cache-control'], 'public, s-maxage=600, max-age=60');
      assert.equal(res.headers.vary, 'Origin, Accept, Accept-Encoding');
      ...
      res = await app.httpRequest().get(`/${pkg.name}/1.0.0/files/tests/integration/test.http.js?meta`);
      assert.equal(res.headers['cache-control'], 'public, s-maxage=600, max-age=60');
      assert.equal(res.headers.vary, 'Origin, Accept, Accept-Encoding');
      assert.equal(res.headers['content-type'], 'application/json; charset=utf-8');
      assert.ok(!res.headers['content-disposition']);
      ...
      res = await app.httpRequest().get(`/${pkg.name}/1.0.0/files/bin/release.sh`);
      assert.equal(res.status, 200);
      assert.equal(res.headers['cache-control'], 'public, max-age=31536000');
      assert.equal(res.headers.vary, 'Origin, Accept, Accept-Encoding');
      ...
      res = await app.httpRequest().get(`/${pkg.name}/1.0.0/files/docs/manifest.appcache`);
      assert.equal(res.status, 200);
      assert.equal(res.headers['cache-control'], 'public, max-age=31536000');
      assert.equal(res.headers.vary, 'Origin, Accept, Accept-Encoding');
    ...
    it('should 404 when version not exists', async () => {
      ...
      let res = await app.httpRequest().get(`/${pkg.name}/1.0.40000404/files/foo.json`).expect(404);
      assert.ok(!res.headers.etag);
      assert.ok(!res.headers['cache-control']);
      assert.equal(res.body.error, `[NOT_FOUND] ${pkg.name}@1.0.40000404 not found`);

      res = await app.httpRequest().get(`/${pkg.name}/1.0.40000404/files/bin/foo/bar.js`).expect(404);
      assert.ok(!res.headers.etag);
      assert.ok(!res.headers['cache-control']);
      assert.equal(res.body.error, `[NOT_FOUND] ${pkg.name}@1.0.40000404 not found`);
    });
