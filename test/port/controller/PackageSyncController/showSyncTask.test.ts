...
describe('test/port/controller/PackageSyncController/showSyncTask.test.ts', () => {
  ...
  describe('[GET /-/package/:fullname/syncs/:taskId] showSyncTask()', () => {
    it('should 401 if user not login when alwaysAuth = true', async () => {
      ...
      const res = await app.httpRequest().get(`/-/package/${pkg.name}/syncs/mock-task-id`).expect(401);
      assert.equal(res.body.error, '[UNAUTHORIZED] Login first');
    });

    it('should 404 when task not exists', async () => {
      const res = await app.httpRequest().get('/-/package/koa/syncs/mock-task-id').expect(404);
    ...
    it('should get sucess task after schedule run', async () => {
      ...
      let res = await app.httpRequest().put(`/-/package/${name}/syncs`).expect(201);
      const taskId = res.body.id;
      assert.ok(taskId);
      res = await app.httpRequest().get(`/-/package/${name}/syncs/${taskId}`).expect(200);
      // waiting state logUrl is not exists
      assert.ok(!res.body.logUrl);
      ...
      res = await app.httpRequest().get(`/-/package/${name}/syncs/${taskId}/log`);
      let log = '';
      if (res.status === 200) {
        log = res.text;
      } else {
      ...
      res = await app.httpRequest().get(`/${name}`).set('accept', 'application/vnd.npm.install-v1+json').expect(200);
  ...
  describe('[GET /:fullname/sync/log/:taskId] deprecatedShowSyncTask()', () => {
    it('should 404 when task not exists', async () => {
      const res = await app.httpRequest().get('/koa/sync/log/mock-task-id').expect(404);
      assert.equal(res.body.error, '[NOT_FOUND] Package "koa" sync task "mock-task-id" not found');
    });
    ...
    it('should 200', async () => {
      ...
      res = await app.httpRequest().get(`/koa/sync/log/${task.taskId}`).expect(200);
      ...
      res = await app.httpRequest().get(`/koa/sync/log/${task.taskId}?t=123`).expect(200);
      assert.ok(res.body.logUrl);
