  ...
  describe('[GET /:fullname/:versionSpec] show()', () => {
    ...
    it('should latest tag with not scoped package', async () => {
      ...
      await app
        ...
        .send(pkg)
        .expect(201);
      let res = await app.httpRequest().get(`/${pkg.name}/latest`).expect(200);
      assert.equal(res.body.version, '2.0.0');

      res = await app.httpRequest().get(`/${pkg.name}/^2.0.0`).expect(200);
      assert.equal(res.body.version, '2.0.0');

      res = await app.httpRequest().get(`/${pkg.name}/%5E2.0.0`).expect(200);
      assert.equal(res.body.version, '2.0.0');
      ...
      res = await app
        ...
        .expect(200);
      res = await app.httpRequest().get(`/${pkg.name}/beta`).expect(200);
      assert.equal(res.body.version, '2.0.0');
