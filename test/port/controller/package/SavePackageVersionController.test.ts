...
describe('test/port/controller/package/SavePackageVersionController.test.ts', () => {
  ...
  describe('[PUT /:fullname] save()', () => {
    it('should set registry field after publish', async () => {
      ...
      const pkg2 = await TestUtil.getFullPackage({
        name: pkg.name,
        version: '2.0.0',
      });
      let res = await app
      ...
      res = await app.httpRequest().get(`/${pkg2.name}`).expect(200);
    ...
    it('should publish 100+ chars length name', async () => {
      ...
      await userRepository.saveUser(user);
      const name =
        '@inrupt/solid-client-0.0.2-dependabotnpmandyarnwebsitedocusauruspreset-classic-200-alpha61-192892303-618.0';
    ...
    it('should add same version throw error', async () => {
      ...
      assert.equal(res.body.ok, true);
      assert.match(res.body.rev, /^\d+-\w{24}$/);
      res = await app.httpRequest().get(`/${pkg.name}/99.0.0`).expect(200);
    ...
    describe('granular token', async () => {
      ...
      it('should 200 add default latest tag', async () => {
        ...
        const queryRes = await app.httpRequest().get(`/${name}/latest`);
