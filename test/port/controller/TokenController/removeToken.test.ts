...
describe('test/port/controller/TokenController/removeToken.test.ts', () => {
  describe('[DELETE /-/npm/v1/tokens/token/:tokenKey] removeToken()', () => {
    it('should 200', async () => {
      ...
      res = await app.httpRequest().get('/-/npm/v1/tokens').set('authorization', authorization).expect(401);
  ...
  describe('[DELETE /-/npm/v1/tokens/gat/:tokenKey] removeGranularToken()', () => {
    ...
    it('should 200', async () => {
      ...
      await app.httpRequest().delete(`/-/npm/v1/tokens/gat/${token.tokenKey}`).expect(204);
